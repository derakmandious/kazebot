...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith('!'):
        ...
        if content.isdigit():
            ...
            numbers_list = [
                int(num.strip()) for num in numbers_str.split(",")
                if num.strip()
            ]
            ...
            if len(numbers_list) > max_images:
                await message.channel.send(
                    f"Please use {max_images} images or less for the GIF.")
                return
...
async def dutchtide_studios_embed(ctx):
    ...
    await ctx.send(embed=embed, view=select, ephemeral=True)
...
async def on_component(ctx):
    if ctx.component_type == 3:  # Select menu
        if ctx.selected_options[0] == "dutchtide_studios":
            ...
            await future_faq_estates_embed(ctx)
